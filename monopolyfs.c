...
void ordrejoueurs(int nbJoueurs)
{
    int *j1=0,*j2=0,*j3=0,*j4=0;
    debutejoueur(&j1,&j2,&j3,&j4,nbJoueurs);
    printf("j1=%d\t,j2=%d\t,j3=%d\t,j4=%d\t",j1,j2,j3,j4);
...
void debutejoueur(int*j1,int*j2,int*j3, int*j4,int nbjoueurs)///passage par adresse avec des pointeurs
{
    srand(time(NULL));              ///on initialise le générateur aléatoire
    int j;
    j=rand()%(nbjoueurs-1+1)+1;     ///nombre aléatoire entre le nbr de joueurs à 1
    if (nbjoueurs==2)               ///si le nbre de joueurs est 2
    {
    ...
    else if (nbjoueurs==3)          /// sinon si le nbre de joueurs est 3
    {
