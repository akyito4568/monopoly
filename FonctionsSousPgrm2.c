...
void affichagepropriete(t_propriete Plateau[32])
{
    ...
    gotoligcol(8, 15);
    do{
        printf("Quelle case voulez vous regarder ");
        scanf("%d",&choixcase);
...
void menu(joueur tab[4], t_propriete Plateau[32], int *nbjoueurs, int* choix){
        int n=0, m=0;
...
void optionsauvegarde(joueur tab[4], t_propriete Plateau[32], int nbjoueurs, char nomsauvegarde[10]){
    ...
    fprintf(sauvegarde, "%s", nomsauvegarde);
    for (i=0; i<nbjoueurs; i++){
        ...
        fprintf(sauvegarde, "%d", tab[i].pos_cord.y);
    }
    for (j=0; j<32; j++){
        fprintf(sauvegarde,"%d", Plateau[j].place);
        ...
        fprintf(sauvegarde,"%d", Plateau[j].y);
    }
    fclose(sauvegarde);
}
